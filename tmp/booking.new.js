...
function initBookingPage(){
  ...
  if(!court || !selectedCourt){
    if(selectedCourt){selectedCourt.innerHTML='<p>Chưa chọn sân. Quay lại <a href="courts.html">Danh sách sân</a>.</p>';} return;
  }
  selectedCourt.innerHTML=`<h2>${court.name}</h2><p>${court.address}</p><div class="court-price">${court.price.toLocaleString()}đ/giờ</div>`;
...
function initPaymentPage(){
  ...
  document.getElementById('payTotal').textContent=`${draft.price.toLocaleString()}đ`;
  ...
  document.getElementById('successAmount').textContent=`${draft.price.toLocaleString()}đ`;
